...
export class ApiService {
  ...
  static async fetchMessages(userWallet: string, peerWallet: string): Promise<any[]> {
    const token = await this.getToken();
    if (!token) {
    ...
    if (!userWallet || !peerWallet) {
      console.error('Invalid wallet addresses - userWallet:', userWallet, 'peerWallet:', peerWallet);
      return [];
    }
  ...
  static async sendMessage(recipientId: string, content: string): Promise<any> {
    const token = await this.getToken();
    if (!token) {
      console.log('User not authenticated');
      return null;
